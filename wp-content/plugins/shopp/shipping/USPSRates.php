 ...
 *
 * $Id: USPSRates.php 22 2012-04-20 18:38:39Z jond $
 **/
...
class USPSRates extends ShippingFramework implements ShippingModule {
	...
	function calculate ($options,$Order) {
		...
		while ($rated = $Estimates->each()) {
			...
			if ('domestic' == $type) {
				$service = $type{0}.$rated->attr(false,'CLASSID');
				$amount = $rated->content('Rate');
				$delivery = false;
			} else {
				$service = $type{0}.$rated->attr(false,'ID');
		...
		while ( $this->packager->packages() ) {
			...
			$_[] = '<Package ID="'.$count++.'">';
				if (false === strpos($type,'IntlRate')) {
					...
					$large = ( max(
						convert_unit($pkg->length(), 'in'),
						convert_unit($pkg->width(), 'in'),
						convert_unit($pkg->height(), 'in')
					) > 12);
					...
					$_[] = '<Container>Rectangular</Container>';
					$_[] = '<Size>'.($large?'LARGE':'REGULAR').'</Size>';

					if ( $pkg->length() + $pkg->width() + $pkg->height() > 0 ) {
						$_[] = '<Width>'.round(convert_unit($pkg->width(), 'in')).'</Width>';
						$_[] = '<Length>'.round(convert_unit($pkg->length(), 'in')).'</Length>';
						$_[] = '<Height>'.round(convert_unit($pkg->height(), 'in')).'</Height>';
						$_[] = '<Girth></Girth>';
					}
